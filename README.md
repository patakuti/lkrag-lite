...
**Token security:** each token is 24 bytes (192 bits) from `crypto.randomBytes`, base64url-encoded — not a derivable ID, not a JWT, and not related in any way to the workspace it grants access to. Only its SHA-256 hash is stored in the database; the plaintext exists solely in `lkragl token create`'s one-time output and in the recipient's `HttpOnly` cookie. A database leak exposes hashes, not usable tokens, and there is nothing in a token or its hash (workspace ID, API key, etc.) to reverse-engineer — brute-forcing 192 bits of randomness from a hash is infeasible.
